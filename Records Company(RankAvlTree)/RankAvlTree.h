...
public:
    ...
    void removeHelper(Node node_to_delete, Node currNode) {
        double extra = currNode->extraGrade;
        if (equal(*(node_to_delete->data), *(currNode->data))) {
            ...
            if (currNode->left_son == nullptr && currNode->right_son == nullptr) {
                ...
                double temp_extra = getExtraSumToMinNode(currNode->right_son, 0);
                delete currNode->data;
    ...
    void addExtraToAllUnder(Node curr, Node toFind, double extra) {
        if (toFind == nullptr) {
            return;
        }
    ...
    double sumOfGrades(Node node, const T &data) {
        double sum = 0;
        if (data == nullptr) {
            return sum;
        }
...
///////////Rank node 
    ...
    double getExtraTo(const T &data){
        if (data == nullptr)
        {
            return 0;
        }
...
//         return NULL;
//     }
void InOrder_e_zeroe() {
       
        Inorde_node_extra(root);
    }

    void Inorde_node_extra(Node node) {
        if (node == nullptr)
            return;
        Inorde_node_extra(node->left_son);
       
        node->extraGrade=0;
        Inorde_node_extra(node->right_son);
    }
